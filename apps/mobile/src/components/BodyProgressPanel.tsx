      ...
      <SafeAreaView style={styles.modalSafe}>
        <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === "ios" ? "padding" : undefined}>
          ...
          <ScrollView contentContainerStyle={styles.modalContent} keyboardDismissMode={Platform.OS === "ios" ? "interactive" : "on-drag"} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.inputs}><View style={styles.field}><Text style={styles.label}>WEIGHT</Text><TextInput value={weight} onChangeText={(value) => setWeight(value.replace(/[^0-9.]/g, ""))} keyboardType="decimal-pad" placeholder="Optional" placeholderTextColor={theme.colors.textMuted} style={styles.input}/></View><View style={styles.field}><Text style={styles.label}>BODY FAT %</Text><TextInput value={bodyFat} onChangeText={(value) => setBodyFat(value.replace(/[^0-9.]/g, ""))} keyboardType="decimal-pad" placeholder="Optional" placeholderTextColor={theme.colors.textMuted} style={styles.input}/></View></View>
            <Pressable accessibilityRole="button" onPress={choosePhoto} style={({ pressed }) => [styles.photoPicker, pressed && styles.pressed]}>{photo ? <Image source={{ uri: photo.uri }} style={styles.photoPreview}/> : <><View style={styles.photoIcon}><Icon name="create" color={theme.colors.brandStrong}/></View><Text style={styles.photoPickerTitle}>Add a progress photo</Text><Text style={styles.photoPickerBody}>Choose a front, side, or back photo from your library.</Text></>}</Pressable>
            <View style={styles.field}><Text style={styles.label}>NOTE</Text><TextInput value={note} onChangeText={setNote} multiline maxLength={500} placeholder="How are you feeling? What changed?" placeholderTextColor={theme.colors.textMuted} style={[styles.input, styles.noteInput]}/></View>
          </ScrollView>
