-- A challenge day is authoritative in the challenge timezone. A member cannot
-- receive tasks or points for a date before their membership began.
...
begin
  ...
  select challenge.status, challenge.starts_on, challenge.ends_on,
         challenge.rules_version, challenge.time_zone
  ...
  scoring_date := (now() at time zone challenge_record.time_zone)::date;
  first_scoring_date := greatest(
    challenge_record.starts_on,
    (member_record.joined_at at time zone challenge_record.time_zone)::date
  );
...
begin
  ...
  select challenge.status, challenge.starts_on, challenge.ends_on, challenge.time_zone
  into challenge_record
  from public.challenges challenge
  where challenge.id = target_challenge_id;
...
begin
  ...
  where challenge.status = 'active'
    and (now() at time zone challenge.time_zone)::date between
      greatest(
        challenge.starts_on,
        (member.joined_at at time zone challenge.time_zone)::date
      )
...
-- Repair existing rows that predate membership. These rows could be created when
-- a device-local date lagged behind the challenge's authoritative timezone.
create temporary table shipshape_scoring_boundaries on commit drop as
select
  ...
  greatest(
    challenge.starts_on,
    (member.joined_at at time zone challenge.time_zone)::date
  ) as first_scoring_date
from public.challenge_members member
join public.challenges challenge on challenge.id = member.challenge_id;

delete from public.score_ledger ledger
using shipshape_scoring_boundaries boundary
where ledger.member_id = boundary.member_id
  and ledger.challenge_id = boundary.challenge_id
  and ledger.effective_date < boundary.first_scoring_date;

delete from public.task_occurrences occurrence
using shipshape_scoring_boundaries boundary
where occurrence.member_id = boundary.member_id
  and occurrence.challenge_id = boundary.challenge_id
  and occurrence.local_date < boundary.first_scoring_date;

delete from public.activity_entries activity
using shipshape_scoring_boundaries boundary
where activity.challenge_id = boundary.challenge_id
  and activity.actor_profile_id = boundary.profile_id
  and activity.event_type = 'day_submitted'
  and activity.metadata ->> 'date' ~ '^\d{4}-\d{2}-\d{2}$'
  and (activity.metadata ->> 'date')::date < boundary.first_scoring_date;

insert into public.domain_event_outbox (topic, event_type, aggregate_id, payload)
select distinct
  'challenge:' || boundary.challenge_id::text || ':activity',
  'score.membership_boundary_repaired',
  boundary.challenge_id,
  jsonb_build_object(
    'version', 1,
    'challengeId', boundary.challenge_id,
    'memberId', boundary.member_id,
    'profileId', boundary.profile_id,
    'firstScoringDate', boundary.first_scoring_date
  )
from shipshape_scoring_boundaries boundary;

comment on function public.list_today_tasks(uuid, date) is
  'Lists tasks for the challenge-authoritative current date and never materializes pre-membership days.';

comment on function public.submit_challenge_day(uuid, date, uuid[]) is
  'Finalizes the challenge-authoritative current date within the member scoring window.';

comment on function public.process_shipshape_daily_scoring() is
  'Materializes and closes only dates within each active member scoring window.';
