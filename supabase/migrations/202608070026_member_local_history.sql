-- History and backfill use the same locked participant timezone as Today and
-- the deadline scorer. Dates before joining and future local dates stay closed.
...
begin
  ...
  select occurrence.id, task.id, task.title, task.instructions, task.task_type,
         task.target_value, task.unit, 1, task.proof_policy,
...
begin
  ...
  select challenge.starts_on, challenge.ends_on,
  ...
  where ledger.challenge_id = target_challenge_id
    and ledger.member_id = member_record.id
    and (
      (ledger.effective_date = target_local_date
       and ledger.entry_type in ('task_complete', 'missed_penalty'))
      or (ledger.effective_date >= target_local_date
          and ledger.entry_type in ('perfect_day', 'streak_bonus'))
    );
  ...
  select occurrence.id,
         ...
         (target_local_date::timestamp + interval '12 hours')
           at time zone member_record.scoring_time_zone
  from public.task_occurrences occurrence
  where occurrence.challenge_id = target_challenge_id
    and occurrence.member_id = member_record.id
    and occurrence.local_date = target_local_date
  ...
  select target_challenge_id, member_record.id, occurrence.id,
         'task_complete'::public.ledger_entry_type, 1, target_local_date,
  ...
  select target_challenge_id, member_record.id, occurrence.id,
         'missed_penalty'::public.ledger_entry_type, -3, target_local_date,
  ...
  update public.task_occurrences occurrence
  set status = case
        when occurrence.id = any(normalized_ids) then
          case when task.proof_policy = 'required'
            then 'pending_review'::public.occurrence_status
            else 'complete'::public.occurrence_status
          end
        else 'missed'::public.occurrence_status
      end,
      completed_at = case
        when occurrence.id = any(normalized_ids)
          then (target_local_date::timestamp + interval '12 hours')
                 at time zone member_record.scoring_time_zone
        else null
      end
  from public.task_definitions task
  ...
  with day_stats as (
    select occurrence.local_date,
           count(*)::integer as task_count,
           bool_and(
             occurrence.status in ('complete', 'pending_review')
           ) as is_perfect
    from public.task_occurrences occurrence
    where occurrence.challenge_id = target_challenge_id
      and occurrence.member_id = member_record.id
  ...
  insert into public.score_ledger (
    challenge_id, member_id, entry_type, points,
    effective_date, idempotency_key, metadata
  )
  select target_challenge_id, member_record.id,
  ...
  with day_stats as (
    ...
    select local_date, task_count,
           row_number() over (
             partition by streak_group order by local_date
           )::integer as streak_number
    from numbered
  )
  insert into public.score_ledger (
    challenge_id, member_id, entry_type, points,
  ...
  select target_challenge_id, member_record.id,
         ...
         streaked.local_date,
         'streak:' || member_record.id::text || ':' || streaked.local_date::text
           || ':' || streaked.streak_number::text,
         jsonb_build_object(
  ...
  insert into public.activity_entries (
    ...
    target_challenge_id, auth.uid(), 'day_submitted', 'updated a past day',
    'challenge',
    jsonb_build_object(
...
comment on function public.list_challenge_history(uuid) is
  'Lists member-local challenge history from the local join day through local today.';
comment on function public.list_challenge_day(uuid, date) is
  'Returns one editable day inside the member-local scoring window.';
comment on function public.amend_challenge_day(uuid, date, uuid[]) is
  'Replaces a member-local day and recalculates authoritative points and bonuses.';
